...
<body class="bg-secondary">
    ...
    <h4 class="text-center text-light p-1">Pilih Foto untuk di Upload</h4>
    <form action="foto.php" method="post" enctype="multipart/form-data">
    <div class="form-group">
    <input type="file" name="foto" class="form-control p-1" required>
    </div>
    <div class="form-group">
    <input type="submit" name="upload" class="btn btn-warning btn-block" value="Upload foto" required>
    </div> 
    <div class="form-group">
  ...
  <table class="table table-striped table-primary table-hover table-sm table-bordered">
          ...
          <th></th>
			...
			<tbody>
				...
				$sql = mysqli_query($koneksi, "SELECT * FROM foto ORDER BY id DESC") or die(mysqli_error($koneksi));
				...
				if(mysqli_num_rows($sql) > 0){
					...
					while($data = mysqli_fetch_assoc($sql)){
						...
						<tr>
							<td>'.$no.'</td>
							<td><image src='.$data['foto'].' width="100px" height="100px"></td>
							<td>
								<a href="delete_foto.php?id='.$data['id'].'" class="badge badge-danger" onclick="return confirm(\'Yakin ingin menghapus data ini?\')">Delete</a>
							</td>
						</tr>
						';
						$no++;
